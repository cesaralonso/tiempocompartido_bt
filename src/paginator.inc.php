...
$domain="http://www.tiempocompartido.com";
/*
 ...
 */
 ...
 if(!isset($_pagi_separador)){
  ...
  if(isset($_pagi_nav_estilo)){
	...
	$_pagi_nav_estilo_mod = "style='padding-right:10px; margin:0 5px; font-size:larger'";
 }else{
 ...
 if(!isset($_pagi_nav_anterior)){
	...
	$_pagi_nav_anterior = "&laquo; Ant";
 } 
 
 if(!isset($_pagi_nav_siguiente)){
	// Si no se ha elegido una cadena para el enlace "siguiente"
	// Se toma la cadena por defecto.
	$_pagi_nav_siguiente = "Sig &raquo;";
 } 
 ...
 */
 ...
 //$_pagi_enlace = $_SERVER['PHP_SELF'];
		...
		$arr_=explode("/",$_SERVER['REQUEST_URI']);
		
		
		foreach($arr_ as $valor => $item){
		if($valor<=3) $urln.=$item."/";
 ...
 foreach($_pagi_propagar as $var){
 	if(isset($GLOBALS[$var])){
		...
		$_pagi_query_string = $domain.$urln;
		//$_pagi_query_string.= $var."=".str_replace(" ","+",$GLOBALS[$var])."&";
	}elseif(isset($_REQUEST[$var])){
		// Si no es global (o register globals está en OFF)
		$_pagi_query_string = $domain.$urln;
		//$_pagi_query_string.= $var."=".str_replace(" ","+",$_REQUEST[$var])."&";
	}
 }
